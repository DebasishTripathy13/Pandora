...
| **Profile** | `eco` / `max` / `test`                                                                                 | `PANDORA_MODEL_PROFILE` |

For each agent, Pandora resolves a tier (from the profile) and walks your AuthMethod priority list, emitting the model identifier that method provides at that tier. The first hit is the primary; **every remaining hit is queued as a fallback in priority order**. langchain's `ModelFallbackMiddleware` walks the queue on primary failure, trying each method in turn until one succeeds.
...
`PANDORA_MODEL_PROFILE` (default: `eco`) controls which tier each agent runs at.
...
| Tier  | Agents                                                                                          |
|-------|-------------------------------------------------------------------------------------------------|
| HIGH  | `pandora`, `exploiter`, `patcher`, `contract_auditor`, `analyst`, `vulnresearch`              |
...
Your inventory is built at startup from environment variables, written by `pandora onboard`.
...
PANDORA_AUTH_PRIORITY=anthropic_oauth,openai_api
...
The factory walks the priority list, drops methods whose detection check fails (placeholder API key, or `PANDORA_AUTH_CLAUDE_CODE=false`), and uses what's left.
...
```
PANDORA_AUTH_PRIORITY=anthropic_api
ANTHROPIC_API_KEY=sk-ant-...
```

| Agent (tier)     | Primary                       | Fallback |
|------------------|-------------------------------|----------|
| pandora (HIGH)| `anthropic/claude-opus-4-7`   | —        |
...
```
PANDORA_AUTH_PRIORITY=openai_api
OPENAI_API_KEY=sk-...
```

| Agent (tier)     | Primary               | Fallback |
|------------------|------------------------|----------|
| pandora (HIGH)| `openai/gpt-5.5`      | —        |
...
| Agent (tier)     | Primary                  | Fallback                         |
|------------------|---------------------------|----------------------------------|
| pandora (HIGH)| `auth/claude-opus-4-7`   | `anthropic/claude-opus-4-7`     |
...
| Agent (tier)     | Primary                       | Fallback                |
|------------------|-------------------------------|-------------------------|
| pandora (HIGH)| `anthropic/claude-opus-4-7`   | `openai/gpt-5.5`        |
...
```
PANDORA_AUTH_PRIORITY=ollama_local
OLLAMA_API_BASE=http://host.docker.internal:11434
OLLAMA_MODEL=qwen3-coder:30b
```
...
| Agent (tier)     | Primary                          | Fallback |
|------------------|----------------------------------|----------|
| pandora (HIGH)| `ollama_chat/qwen3-coder:30b`    | —        |
...
Pandora agents always emit tool calls.

Two probes guard the wiring end-to-end:

1. **`pandora onboard`** — the wizard hits `/api/tags` and
...
2. **litellm container startup** — re-runs the same checks from
   ...
   diagnostic appears in `pandora logs litellm` prefixed with
   `[pandora ollama]`.

### Local Ollama + cloud fallback

```
PANDORA_AUTH_PRIORITY=ollama_local,anthropic_api
OLLAMA_API_BASE=http://host.docker.internal:11434
OLLAMA_MODEL=qwen3-coder:30b
ANTHROPIC_API_KEY=sk-ant-...
```
...
| Agent (tier)     | Primary                          | Fallback                       |
|------------------|----------------------------------|--------------------------------|
| pandora (HIGH)| `ollama_chat/qwen3-coder:30b`    | `anthropic/claude-opus-4-7`    |
...
```
PANDORA_AUTH_PRIORITY=llamacpp_local
...
| Agent (tier)     | Primary                                                  | Fallback |
|------------------|----------------------------------------------------------|----------|
| pandora (HIGH)| `llamacpp/qwen2.5-coder-7b-instruct-q4_k_m`              | —        |
...
```
PANDORA_AUTH_PRIORITY=minimax_api
MINIMAX_API_KEY=eyJ...
```

| Agent (tier)     | Primary                | Notes                                     |
|------------------|------------------------|-------------------------------------------|
| pandora (HIGH)| `minimax/MiniMax-M2.5` | OK                                        |
...
2. Add the model identifier to the appropriate cell of `METHOD_MODELS` in `pandora/llm/models.py`.
...
| `PANDORA_MODEL=<provider/model>` | Registers a global override (e.g. `groq/llama-3.3-70b-versatile`). |
| `PANDORA_MODEL_<ROLE>=<provider/model>` | Per-role override (e.g. `PANDORA_MODEL_RECON=ollama_chat/llama3.2`). |
| `PANDORA_LITELLM_MODELS=<a,b,c>` | Bulk register multiple ids without editing YAML. |
| `CUSTOM_OPENAI_API_BASE` + `CUSTOM_OPENAI_API_KEY` | OpenAI-compatible gateway. Use `custom/<model>` in the override env. |

The proxy logs `[pandora] registered N dynamic model route(s)` at
...
```bash
PANDORA_AUTH_CLAUDE_CODE=true     # Claude subscription
PANDORA_AUTH_CHATGPT=true         # ChatGPT subscription
PANDORA_AUTH_GEMINI=true          # Gemini Advanced
PANDORA_AUTH_COPILOT=true         # Copilot Pro
PANDORA_AUTH_GROK=true            # SuperGrok
PANDORA_AUTH_PERPLEXITY=true      # Perplexity Pro
```
