...
Pandora runs on two Docker networks. Management infrastructure (LLM proxy, databases, agent API) and operational infrastructure (sandbox, C2, targets) are separated so that no offensive tool inside the sandbox can reach the LLM gateway, the API surface, or your credentials over the network. The agent drives the sandbox via the Docker socket, never via TCP.
...
### LiteLLM Proxy (`pandora-net`, port 4000)
...
### LangGraph Platform (`pandora-net`, port 2024)
...
### PostgreSQL (`pandora-net`, port 5432)
...
Two logical databases: `litellm` (managed by LiteLLM) and `pandora_web` (managed via Prisma in the web dashboard).

### Neo4j Knowledge Graph (`sandbox-net` + `pandora-net`, port 7687 / browser 7474)
...
### Web Dashboard (`pandora-net`, port 3000 + terminal WebSocket on 3003)
...
| Credential isolation | Provider API keys live on `pandora-net`; the sandbox never sees them |
