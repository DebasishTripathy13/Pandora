...
Pandora uses [Neo4j](https://neo4j.com/) as a persistent attack graph. Every host, service, vulnerability, credential, and finding discovered during an engagement becomes a typed node with typed relationships. This is the agent's long-term memory across iterations — not conversation history.
...
Default credentials: `neo4j` / `pandora-graph` (set via `NEO4J_PASSWORD`).
...
Agents interact with the graph via tools defined in `pandora/tools/research/tools.py`:
...
Runs `pandora.tools.research.health:main` and reports:
