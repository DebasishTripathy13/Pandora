# Pandora Core Direction: Single Bash Tool + Graph DB Memory

## Why "single bash tool"

Pandora's offensive execution surface should mirror real operators:
...
- `PANDORA_KG_BACKEND=json` (default)
- `PANDORA_KG_PATH=/workspace/kg.json`

Optional Neo4j backend for larger engagements and multi-agent concurrency:

- `PANDORA_KG_BACKEND=neo4j`
- `PANDORA_NEO4J_URI=bolt://localhost:7687`
- `PANDORA_NEO4J_USER=neo4j`
- `PANDORA_NEO4J_PASSWORD=...`
- `PANDORA_NEO4J_DATABASE=neo4j` (optional)
...
Pandora's unique direction is not "chat UX" — it is **autonomous adversary
...
This combination moves Pandora toward a security AGI operator that behaves
