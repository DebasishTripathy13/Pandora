...
### File: `pandora/middleware/opplan.py`
...
| File | Description |
|---|---|
| `pandora/middleware/opplan.py` | OPPLANMiddleware 구현 (5-tool CRUD) |
| `pandora/agents/soundwave.py` | Soundwave 에이전트 (planner.py에서 리네임 + 역할 축소) |
| `pandora/agents/prompts/soundwave.md` | Soundwave 프롬프트 |

#### Modified Files

| File | Change |
|---|---|
| `pandora/middleware/__init__.py` | `OPPLANMiddleware` export 추가 |
| `pandora/agents/pandora.py` | `TodoListMiddleware` → `OPPLANMiddleware`, planner → soundwave |
| `pandora/agents/prompts/pandora.md` | OPPLAN 도구 사용 + Soundwave 역할 반영 |

#### Removed Files

| File | Reason |
|---|---|
| `pandora/agents/planner.py` | `soundwave.py`로 교체 |
| `pandora/agents/prompts/planning.md` | `soundwave.md`로 교체 |

#### Unchanged Files

| File | Reason |
|---|---|
| `pandora/core/schemas.py` | `Objective`, `OPPLAN` 스키마 재사용 (변경 없음) |
| `pandora/agents/recon.py` | OPPLANMiddleware 불필요 (서브에이전트) |
| `pandora/agents/exploit.py` | OPPLANMiddleware 불필요 (서브에이전트) |
| `pandora/agents/postexploit.py` | OPPLANMiddleware 불필요 (서브에이전트) |
...
### Prompt Update: `agents/prompts/pandora.md`
...
| 속성 | Claude Code `TaskCreate` | Pandora `add_objective` |
|---|---|---|
...
| 속성 | Claude Code `TaskGet` | Pandora `get_objective` |
|---|---|---|
...
| 속성 | Claude Code `TaskList` | Pandora `list_objectives` |
|---|---|---|
...
| 속성 | Claude Code `TaskUpdate` | Pandora `update_objective` |
|---|---|---|
...
Pandora은 모든 objective 완료 시 최종 보고서 생성 권고:
