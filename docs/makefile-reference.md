...
The Makefile is for **local development and pre-release verification**. Every Docker target builds from your local checkout. End users install via `curl | bash` and run `pandora` — they don't use `make`.
...
| Target | Description |
|--------|-------------|
| `make dogfood` | Full OSS UX on local code: builds the Go launcher + every service image (`:dev` tag), wires up an isolated `$PANDORA_HOME` under `.dogfood/`, then runs the launcher. The onboard wizard, engagement picker, `compose up`, health checks, and CLI all execute exactly as a `curl \| bash` install. |
...
| `make launcher` | Build the Go launcher binary at `clients/launcher/bin/pandora`. Embedded version is `dev`, which skips release update notices and versioned config sync inside the launcher. Invoked automatically by `make dogfood`. |

`make dogfood` runs against an isolated `.dogfood/` directory so the user's real `~/.pandora` is never touched. `make clean` purges both the compose volumes and `.dogfood/` when you want a fresh onboard.
...
CLI tests are part of `make quality-cli`. To run them in isolation: `npm run test --workspace=@pandora/cli`.
...
| `make web-ee` | Link the Enterprise Edition package (`@pandora/ee`) — dev-only, not part of the OSS flow |
...
| `make health` | KG backend + Neo4j + Web health checks (broader than `pandora kg-health`, which only checks the KG) |
