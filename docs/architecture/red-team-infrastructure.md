# Pandora 레드팀 인프라 아키텍처

> Pandora의 레드팀 인프라 구성과 현업 레드팀 인프라와의 매핑을 정의하는 설계 문서.
...
### 2.1 현업 vs Pandora 대응 관계

Pandora은 통제된 Docker 환경에서 운영되므로 OPSEC 기반 인프라 분리(리다이렉터)는 적용하지 않는다. 단, 팀 서버(C2)는 **별도 컨테이너로 분리**하여 모듈 교체가 가능한 구조를 채택한다.

| 현업 구성 요소 | 현업 역할 | Pandora 대응 | 비고 |
|---|---|---|---|
| **오퍼레이터 (사람)** | 전략 수립, 명령 결정 | **LLM Agent** (Pandora/Recon/Exploit/PostExploit) | 사람 대신 LLM이 판단 |
| **미션 컨트롤** | 작전 계획, 목표 관리 | **Ralph Loop** (Pandora Orchestrator) | opplan.json 기반 자동 오케스트레이션 |
...
| 현업에서 리다이렉터가 필요한 이유 | Pandora에서 불필요한 이유 |
|---|---|
...
graph TB
    subgraph HostMachine["Host Machine"]
        ...
        subgraph PandoraNet["pandora-net (인프라 네트워크)"]
...
graph LR
    ...
    style pandora-net fill:#1a1a2e,stroke:#e94560,color:#fff
    style sandbox-net fill:#0f3460,stroke:#e94560,color:#fff
```

- **pandora-net**: 인프라 서비스 전용. 외부 포트는 `127.0.0.1`에만 바인딩
...
Pandora은 5개 에이전트가 전체 레드팀 킬 체인을 커버한다. 각 단계에서 인프라 사용 방식이 다르다.

```mermaid
graph TD
    subgraph Orchestrator["Pandora Orchestrator (Opus 4.7)"]
...
| 항목 | 값 |
|------|-----|
| 이미지 | `pandora-sandbox` (Kali rolling) |
| 메모리 | 4GB |
| CPU | 2코어 |
| PID 제한 | 1024 |
| 네트워크 | `sandbox-net` |
...
| 항목 | 값 |
|------|-----|
| 이미지 | `pandora-c2-sliver` (Kali rolling) |
| Profile | `c2-sliver` |
| 메모리 | 2GB |
| CPU | 1코어 |
| PID 제한 | 512 |
...
| 서비스 | 포트 | 바인딩 | 네트워크 |
|--------|------|--------|----------|
| LangGraph API | 2024 | 127.0.0.1 | pandora-net |
| LiteLLM Proxy | 4000 | 127.0.0.1 | pandora-net |
| PostgreSQL | 5432 | 127.0.0.1 | pandora-net |
