...
Pandora runs end-to-end on WSL2 with two valid Docker setups:

1. **Docker Desktop with WSL2 backend** (the common path) — Docker Desktop registers `host.docker.internal` automatically.
2. **Native Docker inside the WSL distro** (no Docker Desktop) — Pandora's `docker-compose.yml` adds the `host.docker.internal:host-gateway` mapping itself, so containers reach the host either way.

The default `OLLAMA_API_BASE=http://host.docker.internal:11434` is the right value in **all** environments — including the case where Ollama runs *inside* the same WSL distro as Pandora. From inside a container, `localhost` is the container itself, so it can never reach Ollama on the host. Use `host.docker.internal`.
...
Other WSL caveats:
- Install Pandora under your WSL home (`~/.pandora`), not on a Windows-mounted drive (`/mnt/c/...`) — bind-mounted I/O across the boundary is much slower.
...
```bash
curl -fsSL https://pandora.red/install | bash
```

This downloads the `pandora` CLI binary for your platform and places it in your PATH.
...
`:dev`), with an isolated `$PANDORA_HOME` under `.dogfood/` so your
real `~/.pandora` is untouched.
...
Pandora supports three authentication modes. Choose one during `pandora onboard`.
...
Or edit `~/.pandora/.env` directly:

```bash
PANDORA_AUTH_PRIORITY=anthropic_api,openai_api
...
Run Pandora offline against a local Ollama server — no cloud API
billing, no key.

**Setup:**

1. Install Ollama on your host: <https://ollama.com/download>.
2. Pull a tool-capable model — Pandora agents always call tools, so
   ...
   ```
3. Start the Ollama server bound to all interfaces so the Pandora
...
4. Run `pandora onboard` and pick **"Local LLM (Ollama)"**. The
   wizard prompts for:
   ...
   - `OLLAMA_MODEL` — the wizard probes your running Ollama at the
     ...
     the only valid choice. You cannot type a tag manually: Pandora
...
5. Run `pandora`. A second probe inside the litellm container
...
- `ollama_chat/` (not `ollama/`) routes to Ollama's `/api/chat`
  ...
  every Pandora agent depends on.
- The `ollama_local` AuthMethod collapses HIGH/MID/LOW tiers to the
  ...
  set `PANDORA_AUTH_PRIORITY=ollama_local,anthropic_api` to lead
...
```bash
PANDORA_MODEL_PANDORA=ollama_chat/qwen3-coder:30b   # HIGH agents
PANDORA_MODEL_RECON=ollama_chat/llama3.2:3b             # LOW agents
```
...
3. Configure Pandora to use OAuth:
...
```bash
PANDORA_AUTH_PRIORITY=anthropic_oauth,anthropic_api
PANDORA_AUTH_CLAUDE_CODE=true
PANDORA_MODEL_PROFILE=eco
```
...
- Pandora exposes ChatGPT subscription models as `auth/gpt-5.5`
  and `auth/gpt-5.4`.
- LiteLLM dynamic config maps those aliases through Pandora's custom
...
Set `PANDORA_MODEL` or `PANDORA_LITELLM_MODELS` and Pandora auto-generates the LiteLLM route at container startup:

```bash
PANDORA_MODEL_PROFILE=custom
PANDORA_MODEL=openrouter/anthropic/claude-3.7-sonnet
PANDORA_LITELLM_MODELS=groq/llama-3.3-70b-versatile,together/deepseek-ai/DeepSeek-R1
```
...
| `custom` | Bring your own model via `PANDORA_MODEL` | Varies |
...
```bash
PANDORA_MODEL_RECON=ollama_chat/qwen3-coder:30b
PANDORA_MODEL_EXPLOIT=anthropic/claude-opus-4-7
PANDORA_MODEL_EXPLOIT_TEMPERATURE=0.2
```
...
The web dashboard starts automatically with `pandora` and is accessible at:
...
```bash
pandora                  # Launch platform (all services + interactive CLI)
pandora onboard          # Setup wizard (auth, provider, profile)
pandora onboard --reset  # Re-run setup from scratch
pandora stop             # Stop all services, keep data
pandora status           # Show running services
pandora logs [service]   # Follow service logs
pandora update           # Explicitly refresh config/images and upgrade when available
...
```bash
pandora logs litellm     # LiteLLM proxy logs
pandora logs langgraph   # LangGraph agent logs
pandora logs neo4j       # Knowledge graph logs
pandora kg-health        # Neo4j connection diagnostics
```
...
Configuration saves to `~/.pandora/.env`. Re-run anytime with `pandora onboard --reset`.
...
```bash
# Pandora shows the engagement picker after launch
...
After you approve the OPPLAN, Pandora takes over:
...
```bash
# View findings
ls ~/.pandora/workspace/<engagement>/findings/

# View generated documents
ls ~/.pandora/workspace/<engagement>/plan/
...
cd ~/.pandora && docker compose down -v
```
...
```bash
pandora           # Shows engagement picker → select existing
...
```bash
pandora stop
# Edit ~/.pandora/.env → change PANDORA_AUTH_PRIORITY, PANDORA_AUTH_* toggles, or API keys
pandora
```
...
```bash
pandora status       # Container status (docker compose ps)
pandora kg-health    # Knowledge graph diagnostics (LangGraph + Neo4j connection)
```
...
pandora logs litellm      # LLM proxy
pandora logs neo4j        # Knowledge graph
...
Set `PANDORA_AUTH_CLAUDE_CODE=true` to route Anthropic models through Claude Code OAuth while keeping API-key fallbacks active. The `PANDORA_AUTH_PRIORITY` list controls order:

```bash
PANDORA_AUTH_PRIORITY=anthropic_oauth,anthropic_api,openai_api,google_api
PANDORA_AUTH_CLAUDE_CODE=true      # Primary: Claude via OAuth (auth/* in LiteLLM)
...
The Pandora `auth/` ChatGPT handler reads the Codex CLI credential
...
```bash
# Verify key format
grep _API_KEY ~/.pandora/.env | head -5
...
```bash
pandora stop
cd ~/.pandora && docker compose down -v   # Remove all volumes
pandora onboard --reset                    # Re-run setup
pandora                                    # Fresh start
```
