...
**Pandora 관점**: 스캐너가 아닌 Red Team 에이전트. 목표 지향 운영, OPSEC, 다단계 공격 체인, 조직 대응 검증이 핵심.
...
- **Pandora**: Planning Agent → RoE → CONOPS → OPPLAN → Deconfliction Plan
...
**Pandora 적용**: CONOPS의 위협 행위자 프로필이 ATT&CK 그룹에 매핑되어야 하며, OPPLAN은 에뮬레이션급 TTP 시퀀스를 포함해야 함.
...
2. **단기**: red-run 익스플로잇 스킬 Pandora 포맷 적응
