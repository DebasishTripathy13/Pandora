...
```
pandora/          # Core Python package (LangGraph agents, middleware, tools)
...
└── shared/          # Shared streaming utilities (@pandora/streaming)
...
1. Create `pandora/agents/{name}.py` with a `create_{name}_agent()` factory function
...
Python tests live in `pandora/tests/`. Run inside Docker for a clean environment:
...
When adding a new agent or tool, add corresponding tests in `pandora/tests/`.
