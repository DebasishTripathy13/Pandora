...
- An LLM provider credential — one of:
  ...
  - **Subscription OAuth** (no per-token billing): Claude Max/Pro/Team, ChatGPT Pro/Plus/Team, Gemini Advanced, Copilot Pro, xAI SuperGrok, Perplexity Pro
  - **Other providers** (Groq, Cohere, Together, Fireworks, Perplexity API, Azure, AWS Bedrock, Replicate, custom OpenAI-compatible gateway): supported via `PANDORA_MODEL` / `PANDORA_LITELLM_MODELS` ad-hoc registration
...
This installs the `pandora` CLI to your system.
...
Configuration is saved to `~/.pandora/.env`. Run `pandora onboard --reset` to reconfigure.
...
The web dashboard starts as part of the default stack — it's reachable at `http://localhost:3000` once `pandora` (or `make dev` for contributors) is running.

---

## First Real Engagement

1. Launch Pandora (`pandora`) and open <http://localhost:3000>
...
> **Important**: Only run Pandora against systems you own or have explicit written authorization to test. See the disclaimer in the main README.
...
pandora logs          # Follow LangGraph logs (default)
pandora logs litellm  # Follow a specific service's logs
...
| Contributing to Pandora | [Contributing](contributing.md) |
