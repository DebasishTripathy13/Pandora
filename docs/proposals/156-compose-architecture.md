...
**Issue**: [#156](https://github.com/debasishtripathy13/Pandora/issues/156)
**Author**: VoidChecksum
...
Two `docker-compose.yml` files participate in the running Pandora stack:

| Path | Purpose | Owner |
|---|---|---|
| `~/.pandora/docker-compose.yml` | OSS install (downloaded by `scripts/install.sh` from the release tag) | end user |
...
|---|----------|------------|---------------|
| 1 | CRITICAL | Override placement ambiguity — overrides in `~/.pandora/` are silently ignored when running from the repo dir | **Still real.** The override file at the repo root is auto-merged by Compose any time `docker compose up` is run from the repo. A user who follows OSS docs and edits `~/.pandora/docker-compose.override.yml` (a path that doesn't exist in the install) will see no effect; a contributor who edits the repo `docker-compose.override.yml` will affect dev runs but not OSS-style runs. The semantics depend on `cwd`, which is implicit and surprising. |
...
| 3 | CRITICAL | All 8 services have hardcoded `container_name`, blocking dual-stack | **Still real.** Every service still pins `container_name: pandora-<svc>`. Removing those is a behaviour change for downstream tooling that targets containers by exact name (the launcher binary's `pandora logs`/`stop` use them; user docs and runbooks reference them). Best resolved as part of the architectural decision below, not piecemeal. |
...
| 5 | HIGH | `PANDORA_WORKSPACE_PATH=/workspace` set only in dev override; OSS sandbox missing it | **Already addressed.** The base `docker-compose.yml`'s `sandbox` service binds `${PANDORA_ENGAGEMENT_WORKSPACE:-${PANDORA_HOME:-~/.pandora}/workspace}` to `/workspace`. The env var is implicit through the bind mount, not a separate declaration. |
...
- Document in the new file's preamble that running raw `docker compose up` from the repo will produce the OSS image behaviour, matching `~/.pandora/`.

After this change, `docker compose ls` from the repo (without `make`) reports the same merged config as `docker compose ls` from `~/.pandora/` — the surprise is gone.

#### B. Document the OSS-install path as the user-facing override surface

- Add a section to [`docs/setup-guide.md`](../setup-guide.md) titled "Customizing the install": tell users they can drop a `docker-compose.override.yml` into `~/.pandora/` and Compose will merge it (since the launcher runs Compose from that directory).
...
The launcher binary references containers by exact name (`pandora logs <svc>`, `pandora stop`, `pandora status`). Dropping `container_name` would break those. Two options:

- **D1** (recommended): keep `container_name` for the OSS install, but parameterize via Compose project name. `pandora-${COMPOSE_PROJECT_NAME:-default}-postgres` etc. Default `COMPOSE_PROJECT_NAME=default` reproduces today's names.
