...
# Optional HTTP sandbox daemon — see pandora/sandbox_server/.
#
...
# heavier pandora agent / LLM / langgraph SDKs are deliberately
...
#   - pandora/__init__.py     — package marker (light-weight, just reads __version__)
#   - pandora/sandbox_kernel/ — shared sandbox primitives: TmuxSessionManager,
...
#   - pandora/sandbox_server/ — the FastAPI app + uvicorn entry point.
...
COPY pandora/__init__.py /opt/pandora/__init__.py
COPY pandora/sandbox_kernel /opt/pandora/sandbox_kernel
COPY pandora/sandbox_server /opt/pandora/sandbox_server
ENV PYTHONPATH=/opt
