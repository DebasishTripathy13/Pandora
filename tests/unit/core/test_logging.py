"""Unit tests for pandora.core.logging."""
...
from pandora.core import logging as dlog


@pytest.fixture(autouse=True)
def reset_logger():
    """Reset the pandora root logger between tests."""
    yield
    root = logging.getLogger("pandora")
...
def _capture(level: str | int = "INFO", fmt: str = "text") -> io.StringIO:
    ...
    handler = logging.getLogger("pandora").handlers[0]
    handler.stream = buf
    return buf


class TestGetLogger:
    def test_namespaces_under_pandora(self) -> None:
        log = dlog.get_logger("auth.manager")
        assert log.name == "pandora.auth.manager"
    ...
    def test_auto_configures_on_first_use(self) -> None:
        ...
        assert logging.getLogger("pandora").handlers, "handler should be installed"
        del log
...
class TestJsonFormat:
    def test_json_output_is_parseable(self) -> None:
        ...
        assert record["logger"] == "pandora.test"
        assert "ts" in record
...
class TestConfigureLogging:
    ...
    def test_env_var_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("PANDORA_LOG_FORMAT", "json")
        monkeypatch.setenv("PANDORA_LOG_LEVEL", "DEBUG")
        dlog.configure_logging()
        root = logging.getLogger("pandora")
        assert root.level == logging.DEBUG
        from pandora.core.logging import _JsonFormatter
