"""Unit tests for ``pandora.tools.interaction.ask_user_question``."""
...
from pandora.tools.interaction import ask_user_question
...
def test_emits_custom_event_with_id_and_payload():
    ...
    with (
        patch(
            "pandora.tools.interaction.ask_user.get_stream_writer",
            return_value=fake_writer,
        ),
        patch(
            "pandora.tools.interaction.ask_user.interrupt",
            return_value="Yes",
        ),
    ):
        result = _invoke()
