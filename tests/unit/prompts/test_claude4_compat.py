"""Tests for pandora.agents.prompts.claude4_compat.

Loaded via importlib so the test suite does not require the full
pandora.agents runtime dependency tree (deepagents, langgraph, …).
...
_MODULE_PATH = (
    Path(__file__).resolve().parents[3] / "pandora" / "agents" / "prompts" / "claude4_compat.py"
)
_spec = importlib.util.spec_from_file_location("pandora_claude4_compat_under_test", _MODULE_PATH)
...
class TestPrependNeutralPersona:
    def test_prepends_once(self) -> None:
        prompt = "PANDORA: autonomous red team orchestrator."
...
class TestApplyClaude4Compat:
    ...
    def test_env_disable(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("PANDORA_CLAUDE4_COMPAT", "0")
