"""Unit tests for pandora.llm.factory."""
...
from pandora.llm.factory import (
    LLMFactory,
    _is_real_key,
...
class TestOAuthCredentialsPresent:
    ...
    Without the file check, ``PANDORA_AUTH_CLAUDE_CODE=true`` plus a
...
class TestLLMFactory:
    ...
    def test_get_model_different_roles_different_models(self):
        ...
        pandora = self.factory.get_model("pandora")
        assert recon is not pandora
        assert recon.model_name != pandora.model_name
    ...
    def test_get_fallback_models_high_tier_includes_all_methods(self):
        models = self.factory.get_fallback_models("pandora")
        names = [m.model_name for m in models]
        assert names == [
            "openai/gpt-5.5",
    ...
    def test_explicit_credentials_param(self):
        ...
        assert factory.get_model("pandora").model_name == "openai/gpt-5.5"
...
class TestResolveCredentials:
    def test_real_keys_only(self, monkeypatch):
        ...
        monkeypatch.delenv("PANDORA_AUTH_PRIORITY", raising=False)
        monkeypatch.delenv("PANDORA_AUTH_CLAUDE_CODE", raising=False)
        for flag in (
            "PANDORA_AUTH_CHATGPT",
            "PANDORA_AUTH_COPILOT",
            "PANDORA_AUTH_GEMINI",
            "PANDORA_AUTH_GROK",
            "PANDORA_AUTH_PERPLEXITY",
        ):
    ...
    def test_oauth_only(self, monkeypatch, tmp_path):
        ...
        for k in (
            ...
            monkeypatch.delenv(k, raising=False)
        monkeypatch.setenv("PANDORA_AUTH_CLAUDE_CODE", "true")
        monkeypatch.delenv("PANDORA_AUTH_PRIORITY", raising=False)
        for flag in (
            "PANDORA_AUTH_CHATGPT",
            "PANDORA_AUTH_COPILOT",
    ...
    def test_oauth_flag_without_credential_file_is_dropped(self, monkeypatch, tmp_path):
        """Stale ``PANDORA_AUTH_CLAUDE_CODE=true`` after ``codex logout``
    ...
    def test_explicit_priority_override(self, monkeypatch):
        monkeypatch.setenv("PANDORA_AUTH_PRIORITY", "openai_api,anthropic_api")
        monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-api03-realtokenfortestingauthrouting12345")
        monkeypatch.setenv("OPENAI_API_KEY", "sk-proj-realopenaitokenfortestingauthrouting12345")
    ...
    def test_explicit_priority_no_creds_logs_error(self, monkeypatch, tmp_path, caplog):
        """An explicit ``PANDORA_AUTH_PRIORITY`` whose every listed
        ...
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("CLAUDE_CODE_CREDENTIALS_PATH", str(missing))
        monkeypatch.setenv("PANDORA_AUTH_PRIORITY", "anthropic_oauth")
        monkeypatch.setenv("PANDORA_AUTH_CLAUDE_CODE", "true")
        for k in (
            "ANTHROPIC_API_KEY",
            "OPENAI_API_KEY",
        ...
        # ``pandora.core.logging`` sets the parent pandora logger
        ...
        pandora_log = logging.getLogger("pandora")
        monkeypatch.setattr(pandora_log, "propagate", True)
        ...
        # scanning ``pandora logs langgraph`` knows where to look.
        error_records = [r for r in caplog.records if r.levelname == "ERROR"]
        assert any("PANDORA_AUTH_PRIORITY" in r.getMessage() for r in error_records), (
            f"No ERROR log mentioned PANDORA_AUTH_PRIORITY. "
            f"Got: {[r.getMessage() for r in error_records]}"
        )

    def test_implicit_priority_no_creds_stays_info_level(self, monkeypatch, caplog):
        """Implicit priority (no PANDORA_AUTH_PRIORITY set) with no
        ...
        # records — the pandora parent logger defaults to
        # ``propagate=False`` per ``pandora.core.logging``.
        pandora_log = logging.getLogger("pandora")
        monkeypatch.setattr(pandora_log, "propagate", True)
    ...
    def test_ollama_local_only_returns_ollama_chain(self, monkeypatch):
        ...
        for k in (
            ...
            "OPENROUTER_API_KEY",
            "NVIDIA_API_KEY",
            "PANDORA_AUTH_PRIORITY",
            "PANDORA_AUTH_CLAUDE_CODE",
            "PANDORA_AUTH_CHATGPT",
        ):
            monkeypatch.delenv(k, raising=False)
    ...
    def test_explicit_priority_with_ollama_local(self, monkeypatch):
        ...
        monkeypatch.setenv("PANDORA_AUTH_PRIORITY", "ollama_local,anthropic_api")
        monkeypatch.setenv("OLLAMA_API_BASE", "http://host.docker.internal:11434")
        monkeypatch.setenv("OLLAMA_MODEL", "qwen3-coder:30b")
        monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-api03-realtokenfortestingauthrouting12345")
        for k in (
...
class TestTemperatureDrop:
    ...
    def setup_method(self):
        from pandora.llm.factory import _model_drops_temperature
...
class TestActionableErrorTranslation:
    ...
    def setup_method(self):
        from pandora.llm.factory import _reraise_with_actionable_message
    ...
    def test_no_fallback_model_group_branch(self):
        ...
        assert "PANDORA_AUTH_PRIORITY" in msg
    ...
    def test_401_authentication_branch(self):
        ...
        assert "pandora onboard --reset" in msg
...
class TestDeepSeekReasoningContent:
    ...
    def test_create_chat_result_extracts_reasoning_content(self):
        ...
        from pandora.llm.factory import _DeepSeekThinkingChatOpenAI
    ...
    def test_model_detection(self):
        ...
        """
        from pandora.llm.factory import _model_is_deepseek_thinking
...
class TestResolveCredentialsForLlamacpp:
    ...
    def test_priority_list_with_llamacpp_picks_it_up(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """User wrote an explicit PANDORA_AUTH_PRIORITY listing
        llamacpp_local — the resolver must respect that ordering."""
        _scrub_other_credentials(monkeypatch)
        monkeypatch.setenv("LLAMACPP_API_BASE", "http://localhost:8080/v1")
        monkeypatch.setenv("LLAMACPP_MODEL", "qwen-coder")
        monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-api03-realtokenfortestingauthrouting12345")
        monkeypatch.setenv("PANDORA_AUTH_PRIORITY", "llamacpp_local,anthropic_api")
