"""Unit tests for pandora.llm.router — thin wrapper over LLMModelMapping."""
...
class TestModelRouter:
    ...
    def test_resolve_pandora_is_high_tier(self):
        assert self.router.resolve("pandora") == "anthropic/claude-opus-4-7"
    ...
    def test_resolve_with_fallback_high_tier_full_chain(self):
        chain = self.router.resolve_with_fallback("pandora")
        assert chain == [
    ...
    def test_resolve_with_single_credential_no_fallback(self):
        ...
        chain = router.resolve_with_fallback("pandora")
