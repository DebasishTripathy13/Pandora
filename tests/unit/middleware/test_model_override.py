...
from pandora.middleware.model_override import (
...
def proxy_env(monkeypatch: pytest.MonkeyPatch) -> dict[str, str]:
    ...
    pandora-net Docker network; the bare ``ProxyConfig()`` default of
    ``localhost:4000`` does not work inside a container — issue #186.
    """
    env = {
        "PANDORA_LLM__PROXY_URL": "http://litellm:4000",
        "PANDORA_LLM__PROXY_API_KEY": "sk-pandora-master",
        "PANDORA_LLM__TIMEOUT": "120",
        "PANDORA_LLM__MAX_RETRIES": "2",
    }
...
class TestBuildProxiedLLMResolvesConfig:
    ...
    def test_uses_resolved_proxy_url_from_env(self, proxy_env: dict[str, str]) -> None:
        """Override must hit the resolved ``PANDORA_LLM__PROXY_URL``,
        ...
        assert bound.openai_api_base == proxy_env["PANDORA_LLM__PROXY_URL"], (
    ...
    def test_uses_resolved_api_key_from_env(self, proxy_env: dict[str, str]) -> None:
        ...
        Bare default would be ``sk-pandora-master`` which happens to
        ...
        assert api_key == proxy_env["PANDORA_LLM__PROXY_API_KEY"]
    ...
    def test_proxy_url_default_falls_through_when_env_unset(
        ...
        for var in (
            "PANDORA_LLM__PROXY_URL",
            "PANDORA_LLM__PROXY_API_KEY",
            "PANDORA_LLM__TIMEOUT",
            "PANDORA_LLM__MAX_RETRIES",
        ):
