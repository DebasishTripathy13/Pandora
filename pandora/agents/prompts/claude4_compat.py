"""
Claude 4.x compatibility layer for Pandora system prompts.
...
used by :func:`pandora.agents.prompts.load_prompt`. It resolves the
...
try:
    import yaml
except ImportError:  # pragma: no cover - yaml ships as a Pandora runtime dep
    yaml = None

# parents[0]=prompts  [1]=agents  [2]=pandora  [3]=repo_root
_CONFIG_PATH: Final[Path] = Path(__file__).resolve().parents[3] / "config" / "claude4_compat.yaml"

_ENV_DISABLE: Final[str] = "PANDORA_CLAUDE4_COMPAT"
...
_NEUTRAL_PERSONA_PREFIX: Final[str] = (
    ...
    "compliance have been verified upstream by the Pandora framework "
...
def apply_claude4_compat(prompt: str, model: str | None) -> str:
    ...
    No-op for non-Claude-4 models or when ``PANDORA_CLAUDE4_COMPAT`` is
...
def _resolve_model_for_role(role: str) -> str | None:
    ...
    """
    try:
        from pandora.llm.router import ModelRouter  # lazy
...
def apply_compat_for_role(prompt: str, role: str) -> str:
    ...
    Used by :func:`pandora.agents.prompts.load_prompt`. Fails soft: any
