...
2. **Tool prompt co-location** — tool prompts live with their tool code (e.g.,
   pandora/tools/bash/prompt.py) and are injected here with role-specific variations.
...
def _get_tool_prompt(tool_name: str, role: str | None = None) -> str:
    ...
    """
    if tool_name == "bash":
        from pandora.tools.bash.prompt import BASH_PROMPT
...
# The orchestrator (pandora) and soundwave don't do direct tool execution,
...
def load_prompt(name: str, *, shared: list[str] | None = None) -> str:
    ...
    # per-agent edits.
    #
    # PANDORA_LANGUAGE env var pins the output language (e.g. "en", "ko",
    ...
    # Runtime language pin: when PANDORA_LANGUAGE is set, replace the
    ...
    pinned_lang = os.environ.get("PANDORA_LANGUAGE", "").strip()
    ...
    # See pandora/agents/prompts/claude4_compat.py and docs/model-compatibility.md.
    try:
        from pandora.agents.prompts.claude4_compat import apply_compat_for_role
