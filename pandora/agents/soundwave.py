...
Named after the Pandora intelligence officer who intercepts, processes,
...
from pandora.middleware import EngagementContextMiddleware, FilesystemMiddleware
from pandora.middleware.skills import SkillsMiddleware
from pandora.tools.interaction import ask_user_question, complete_engagement_planning
...
def create_soundwave_agent():
    ...
    # VM), HTTPSandbox when PANDORA_FILESYSTEM_BACKEND=http (Cloud Run
    # multi-container deploys where there's no host docker daemon). See
    # pandora/backends/factory.py for the env contract.
