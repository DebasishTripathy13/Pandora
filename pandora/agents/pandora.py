"""Pandora — autonomous red team coordinator agent.
...
def create_pandora_agent():
    """Initialize the Pandora Orchestrator using create_agent() directly.
    ...
    # when PANDORA_FILESYSTEM_BACKEND=http (see backends/factory.py).
    sandbox = build_sandbox_backend(config.docker.sandbox_container_name)

    system_prompt = load_prompt("pandora")
    ...
    # pandora delegation. Document edits while docs already exist are
    # handled by pandora's FilesystemMiddleware directly.
    subagents = [
        CompiledSubAgent(
            name="recon",
            description=(
    ...
    from pandora.middleware.model_override import ModelOverrideMiddleware

    middleware = [
        EngagementContextMiddleware(),
        SkillsMiddleware(
            backend=backend,
            sources=["/skills/pandora/", "/skills/shared/", *benchmark_skill_sources()],
        ),
