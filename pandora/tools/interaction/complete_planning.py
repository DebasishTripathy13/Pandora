"""complete_engagement_planning — signal soundwave-to-pandora handoff.
...
``soundwave`` to ``pandora`` so the next operator message lands on the
...
def complete_engagement_planning(
    ...
    """Signal that engagement planning is finished and hand off to Pandora.
    ...
    their schemas. The CLI will switch the active assistant to Pandora and
