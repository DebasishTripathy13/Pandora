"""Skill loading tools for Pandora agents."""
...
def _unwrap_backend(backend: Any) -> Any:
    ...
    from pandora.middleware.filesystem import EngagementFilesystemBackend  # noqa: PLC0415
...
# ── load_skill tool ──────────────────────────────────────────────────────────
# A Pandora-specific replacement for `load_skill("/skills/...")` that
...
def build_load_skill_tool(backend: Any, sources: list[str]):  # type: ignore[no-untyped-def]
    ...
    def load_skill(skill_path: str, include_siblings: bool = False) -> str:
        """Load a Pandora skill file (full body, no line-limit truncation).
