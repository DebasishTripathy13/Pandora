"""Neo4j persistence backend for :mod:`pandora.research.graph`.
...
``pandora.research._state``:

- ``PANDORA_KG_BACKEND=neo4j``
- ``PANDORA_NEO4J_URI``
- ``PANDORA_NEO4J_USER``
- ``PANDORA_NEO4J_PASSWORD``
- ``PANDORA_NEO4J_DATABASE`` (optional, default: ``neo4j``)
...
from pandora.tools.research.graph import Edge, EdgeKind, KnowledgeGraph, Node, NodeKind
...
class Neo4jConfig:
    ...
    def from_env(cls) -> Neo4jConfig:
        uri = os.environ.get("PANDORA_NEO4J_URI", "").strip()
        user = os.environ.get("PANDORA_NEO4J_USER", "").strip()
        password = os.environ.get("PANDORA_NEO4J_PASSWORD", "").strip()
        database = os.environ.get("PANDORA_NEO4J_DATABASE", "neo4j").strip() or "neo4j"

        missing: list[str] = []
        if not uri:
            missing.append("PANDORA_NEO4J_URI")
        if not user:
            missing.append("PANDORA_NEO4J_USER")
        if not password:
            missing.append("PANDORA_NEO4J_PASSWORD")
