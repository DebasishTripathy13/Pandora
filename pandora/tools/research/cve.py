...
def _default_cache_path() -> Path:
    ...
    Override via ``PANDORA_CVE_CACHE`` for tests or alternate
    deployments.
    """
    override = os.environ.get("PANDORA_CVE_CACHE")
    if override:
        return Path(override)
    return Path.home() / ".pandora" / "cache" / "cve.json"
...
def _resolve_poc_links(cve_id: str) -> list[str]:
    ...
    ``pandora.references`` is not yet on the path (e.g. smaller
