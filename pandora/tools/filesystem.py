"""Filesystem tool-surface helpers for Pandora middleware."""
...
def filesystem_tools_without_execute(tools: Sequence[Any]) -> list[Any]:
    ...
    Pandora routes command execution through its dedicated bash tools, while
    Deep Agents' filesystem middleware owns file operations. Keeping this
    selection helper in ``pandora.tools`` makes the public tool surface
