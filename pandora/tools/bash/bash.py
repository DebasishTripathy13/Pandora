"""Bash tool for the Pandora agent.

Thin wrapper around DockerSandbox.execute_tmux(). All tmux session
management and PS1 polling logic lives in pandora/backends/docker_sandbox.py.
...
from pandora.backends.docker_sandbox import DockerSandbox
from pandora.sandbox_kernel.tmux import _interpret_exit_code

log = logging.getLogger("pandora.tools.bash.bash")

_sandbox: DockerSandbox | None = None
_current_workspace_path: contextvars.ContextVar[str] = contextvars.ContextVar(
    "pandora_bash_workspace_path",
    default="/workspace",
)
...
def _workspace_path_from_config(config: RunnableConfig | None) -> str:
    ...
    env_workspace = os.environ.get("PANDORA_WORKSPACE_PATH")
    if env_workspace:
        normalized = DockerSandbox._normalize_workspace_path(env_workspace)
        if normalized != "/workspace":
            return normalized

    env_slug = os.environ.get("PANDORA_ENGAGEMENT")
    if env_slug:
