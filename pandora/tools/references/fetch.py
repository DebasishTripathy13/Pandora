...
cache under ``~/.pandora/references/<slug>/`` (override via
``PANDORA_REFERENCES_ROOT``) so agents can grep through the full
...
from pandora.tools.references.catalog import REFERENCES, ReferenceEntry


def _default_cache_root() -> Path:
    override = os.environ.get("PANDORA_REFERENCES_ROOT")
    if override:
        return Path(override)
    return Path.home() / ".pandora" / "references"
