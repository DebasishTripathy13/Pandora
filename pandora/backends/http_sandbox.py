...
`pandora.sandbox_server`, a FastAPI app that runs *inside* the
...
from pandora.sandbox_kernel import BackgroundJob, BackgroundJobTracker


class HTTPSandbox(BaseSandbox):
    """A `BaseSandbox` that talks to a `pandora.sandbox_server` daemon.
    ...
    def _http(self) -> httpx.Client:
        ...
        if self._client is None:
            headers = {"User-Agent": "pandora-http-sandbox/1"}
            if self._token:
    ...
    # `pandora/tools/bash/bash.py:_sandbox.execute_tmux_async(...)`
