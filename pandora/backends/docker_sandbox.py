...
``pandora.sandbox_kernel.base.SandboxBase`` — the shared implementation
...
the daemon (``pandora.sandbox_server``) imports ``LocalSandbox`` from
``sandbox_kernel.local`` and never sees ``DockerSandbox``. The
``backends/`` package stays an agent-only namespace, mirroring the
pre-refactor boundary where the sandbox image shipped zero pandora
...
``from pandora.backends.docker_sandbox import TmuxSessionManager``
...
from pandora.sandbox_kernel import (
...
from pandora.sandbox_kernel.base import SandboxBase
...
log = logging.getLogger("pandora.backends.docker_sandbox")
...
class DockerSandbox(SandboxBase):
    ...
    def __init__(
        self,
        container_name: str = "pandora-sandbox",
...
def check_sandbox_running(container_name: str = "pandora-sandbox") -> bool:
