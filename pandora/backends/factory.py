...
Pool-plane agents (Soundwave + the Pandora orchestrator) use only file
...
from pandora.backends.http_sandbox import HTTPSandbox
...
def build_sandbox_backend(container_name: str):
    ...
    """
    kind = (os.environ.get("PANDORA_FILESYSTEM_BACKEND") or "docker").strip().lower()
    if kind == "http":
