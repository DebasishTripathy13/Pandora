...
Pattern matches OPPLANMiddleware (pandora/middleware/opplan.py) —
...
from pandora.middleware.opplan import _reduce_engagement_name, _reduce_workspace_path
from pandora.tools.bash.bash import bash_workspace
...
class EngagementContextState(AgentState):
    ...
    # the prompt-time PANDORA_LANGUAGE env policy. Multi-tenant launchers
...
class EngagementContextMiddleware(AgentMiddleware):
    ...
    def _inject(self, request):
        ...
        # produced if PANDORA_LANGUAGE were set, but per-run rather than
        ...
        from pandora.agents.prompts import build_language_policy
