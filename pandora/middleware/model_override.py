...
LLM untouched (the ``factory.get_model("pandora")`` baked in at
agent construction).
...
from pandora.llm.factory import LLMFactory, _model_drops_temperature
...
def _build_proxied_llm(model_id: str, original: BaseChatModel) -> BaseChatModel:
    ...
    so we honour ``PANDORA_LLM__PROXY_URL`` (e.g. ``http://litellm:4000``
