...
plus the OAuth toggle) and respects ``PANDORA_AUTH_PRIORITY`` for
...
from pandora.core.logging import get_logger
...
# Default ordering when PANDORA_AUTH_PRIORITY is not set. OAuth methods
...
_OAUTH_METHOD_ENV: dict[AuthMethod, str] = {
    AuthMethod.ANTHROPIC_OAUTH: "PANDORA_AUTH_CLAUDE_CODE",
    AuthMethod.OPENAI_OAUTH: "PANDORA_AUTH_CHATGPT",
    AuthMethod.GOOGLE_OAUTH: "PANDORA_AUTH_GEMINI",
    AuthMethod.COPILOT_OAUTH: "PANDORA_AUTH_COPILOT",
    AuthMethod.GROK_OAUTH: "PANDORA_AUTH_GROK",
    AuthMethod.PERPLEXITY_OAUTH: "PANDORA_AUTH_PERPLEXITY",
}
...
# ``PANDORA_AUTH_CLAUDE_CODE=true`` are intent (the user enabled the
...
def _oauth_credentials_present(method: AuthMethod) -> bool:
    ...
    honest — without it, ``PANDORA_AUTH_CLAUDE_CODE=true`` plus a
...
def _resolve_credentials() -> Credentials:
    ...
    Walks ``PANDORA_AUTH_PRIORITY`` (comma-separated AuthMethod
    ...
    """
    priority_raw = os.getenv("PANDORA_AUTH_PRIORITY", "")
    ...
    if priority_explicit:
        ...
        for token in priority_raw.split(","):
            ...
            try:
                ...
                log.warning("Unknown method in PANDORA_AUTH_PRIORITY: %s", token)
    else:
    ...
    if not methods:
        ...
        if priority_explicit:
            # User expressed clear intent (set PANDORA_AUTH_PRIORITY) but
            ...
            log.error(
                "PANDORA_AUTH_PRIORITY=%r set but no listed method has "
                ...
                "(e.g. PANDORA_AUTH_CLAUDE_CODE=true requires "
...
def _reraise_if_connection_error(exc: Exception) -> None:
    ...
    if any(
        ...
        raise RuntimeError(
            ...
            f"Check 'pandora logs litellm' for details."
        ) from exc
...
def _reraise_with_actionable_message(exc: Exception, model_name: str) -> None:
    ...
    if "no fallback model group found" in msg_lower:
        raise RuntimeError(
            ...
            f"PANDORA_AUTH_PRIORITY or fix the upstream error.\n"
            f"Underlying: {msg}"
        ) from exc
    ...
    if (
        ...
        raise RuntimeError(
            ...
            f"Check the API key for that provider in ~/.pandora/.env, "
            f"or run 'pandora onboard --reset'.\nUnderlying: {msg}"
        ) from exc

    if "ratelimit" in err_type.lower() or "code: 429" in msg_lower:
        raise RuntimeError(
            f"Model '{model_name}' hit the provider's rate limit (429). "
            f"Add another method to PANDORA_AUTH_PRIORITY so the agent "
...
class LLMFactory:
    ...
    PandoraConfig (env: ``PANDORA_MODEL_PROFILE``).
    """
    ...
    def _resolve_proxy_config() -> ProxyConfig:
        """Resolve proxy config from PandoraConfig (env vars)."""
    ...
    def _resolve_profile() -> ModelProfile:
        """Resolve the model profile from PandoraConfig (env var)."""
        from pandora.core.config import load_config
