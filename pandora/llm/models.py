...
User has [anthropic_api, openai_api]. Profile=eco.
  pandora (HIGH) → primary=anthropic/claude-opus-4-7, fallback=openai/gpt-5.5
...
User has [anthropic_oauth, anthropic_api]. Profile=eco.
  pandora (HIGH) → primary=auth/claude-opus-4-7, fallback=anthropic/claude-opus-4-7
  (OAuth subscription primary, paid API as fallback when subscription quota hits.)

User has [anthropic_oauth] only. Profile=eco.
  pandora (HIGH) → primary=auth/claude-opus-4-7, fallback=None

User has [openai_api]. Profile=eco.
  pandora (HIGH) → primary=openai/gpt-5.5, fallback=None
...
``PANDORA_MODEL_<ROLE>`` to one of the registered Codex variant routes:
...
class AuthMethod(StrEnum):
    ...
    COPILOT_OAUTH = "copilot_oauth"  # Copilot Pro subscription
...
METHOD_MODELS: dict[AuthMethod, dict[Tier, str]] = {
    ...
    AuthMethod.OLLAMA_LOCAL: {
        ...
        # endpoint that supports tool/function calling, which Pandora
    ...
    AuthMethod.COPILOT_OAUTH: {
        ...
        # can be selected per-agent via ``PANDORA_MODEL_<ROLE>``.
    ...
    AuthMethod.VERTEX_API: {
        ...
        # snapshot is needed. Override per-role via PANDORA_MODEL_<ROLE>
    ...
    # naming; users can override per role via PANDORA_MODEL_<ROLE>.
    ...
    # the OpenAI tools schema and Pandora agents always emit tool
    # calls. Override via PANDORA_MODEL_<ROLE> for non-tool roles.
    ...
    # endpoint which silently drops the ``tools`` parameter; Pandora
...
def _resolve_ollama_model() -> str | None:
    ...
    one that exposes tool/function calling, which every Pandora agent
    relies on.
