...
docker-less ``DaemonSandbox`` class. Everything else hangs off of it:
  ...
  ``pandora.backends.docker_sandbox.DockerSandbox`` — agent-side,
      uses ``docker exec`` as transport. Imports ``TmuxSessionManager``
      from this package via the ``exec_prefix=["docker", "exec", ...]``
      configuration knob.

  ``pandora.backends.http_sandbox.HTTPSandbox`` — agent-side,
  ...
  ``pandora.sandbox_server.app`` — sandbox-side, FastAPI daemon.
...
zero pandora Python code. Adding the daemon to the same image
...
from pandora.sandbox_kernel.jobs import BackgroundJob, BackgroundJobTracker
from pandora.sandbox_kernel.tmux import (
