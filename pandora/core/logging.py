"""Pandora logging — stdlib wrapper with optional structured JSON output.
...
Toggle via env vars (read once at import / first ``configure_logging`` call):
    ...
    PANDORA_LOG_LEVEL    DEBUG | INFO (default) | WARNING | ERROR
    PANDORA_LOG_FORMAT   text (default) | json
...
def configure_logging(
    ...
    """Configure the root ``pandora`` logger.

    Idempotent: calling twice replaces the prior handler so tests can
    reconfigure freely. Honors env vars when args are omitted.
    """
    global _CONFIGURED
    level = level or os.getenv("PANDORA_LOG_LEVEL", "INFO")
    fmt = (fmt or os.getenv("PANDORA_LOG_FORMAT", "text")).lower()
