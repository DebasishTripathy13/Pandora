"""Pandora configuration — defaults + environment variable overrides.

LLM model assignments are defined in pandora.llm.models (LLMModelMapping).
...
by ``pandora.llm.factory._resolve_credentials`` directly from environment
variables (``ANTHROPIC_API_KEY`` etc., ``PANDORA_PROVIDER_PRIORITY``,
``PANDORA_AUTH_CLAUDE_CODE``) and so don't appear in this schema.
"""
...
from pandora.llm.models import ModelProfile
...
class LLMConfig(BaseModel):
    ...
    proxy_api_key: str = "sk-pandora-master"
    timeout: int = 120
    max_retries: int = 2
...
class DockerConfig(BaseModel):
    ...
    nested env vars, e.g. ``PANDORA_DOCKER__POLL_INTERVAL=0.25``.
    """

    sandbox_container_name: str = "pandora-sandbox"
    sandbox_image: str = "pandora-sandbox:latest"
    network: str = "pandora-net"
...
class PandoraConfig(BaseSettings):
    """Root configuration.

    Set PANDORA_MODEL_PROFILE to switch tier presets:
    ...
    Provider routing is driven by environment variables, not this schema:
      PANDORA_PROVIDER_PRIORITY  comma-separated provider order
                                    (default: anthropic,openai,google,minimax)
      PANDORA_AUTH_CLAUDE_CODE   "true" → route Anthropic via OAuth
      ANTHROPIC_API_KEY / OPENAI_API_KEY / GEMINI_API_KEY / MINIMAX_API_KEY
                                    detected by the LLM factory; placeholder
                                    values are ignored.
    """

    model_config = {"env_prefix": "PANDORA_", "env_nested_delimiter": "__"}
