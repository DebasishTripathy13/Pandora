...
The checked-in ``config/litellm.yaml`` contains the default Pandora routes.
Operators can additionally set ``PANDORA_MODEL`` / per-role overrides to any
...
``supports_function_calling`` check, and Pandora agents always emit tool calls.
...
PROVIDER_API_KEY_ENV: dict[str, str] = {
    ...
    # operators set ``PANDORA_LITELLM_MODELS=xiaomi_mimo/<id>`` and
...
def _looks_like_model_env_var(name: str) -> bool:
    if name in {"PANDORA_MODEL", "PANDORA_MODEL_FALLBACK"}:
        return True
    if not name.startswith("PANDORA_MODEL_"):
        return False
    suffix = name.removeprefix("PANDORA_MODEL_")
...
def collect_requested_models(env: Mapping[str, str] | None = None) -> set[str]:
    """Collect model IDs requested through PANDORA_MODEL* env vars.
    ...
    models.update(_extra_models_from_env(source.get("PANDORA_LITELLM_MODELS")))
...
def validate_model_name(model_name: str) -> None:
    ...
    matching ``PANDORA_AUTH_*`` flag is set, with custom-provider dispatch
    ...
    ``PANDORA_MODEL=auth/gpt-5.4-mini`` plus ``PANDORA_AUTH_CHATGPT=true``
    ...
    if provider == "ollama":
        ...
        raise ValueError(
            ...
            "calling. Pandora agents always emit tool calls — use "
...
def build_model_entry(model_name: str) -> dict[str, Any]:
    ...
    if provider == "custom":
        ...
        #   PANDORA_MODEL=custom/qwen3-coder
...
# becomes unhealthy. Gating on PANDORA_AUTH_* prevents that.
...
_SUBSCRIPTION_ROUTES: dict[str, list[dict[str, Any]]] = {
    ...
    "PANDORA_AUTH_CHATGPT": [
        ...
        # like ``PANDORA_MODEL_PATCHER=auth/gpt-5.3-codex`` work
    ...
    "PANDORA_AUTH_COPILOT": [
        ...
        # ``PANDORA_MODEL_<ROLE>=copilot/gpt-5.3-codex`` for
...
def _inject_subscription_routes(
    ...
    Only registers routes for providers whose ``PANDORA_AUTH_*`` flag is
...
def has_subscription_routes(env: Mapping[str, str] | None = None) -> bool:
    """Return True if any PANDORA_AUTH_* flag enables a subscription route.

    Used by ``litellm_startup.py`` to decide whether to regenerate the
    LiteLLM config even when no ``PANDORA_MODEL*`` overrides are set —
    a user who only enables ``PANDORA_AUTH_CHATGPT=true`` still needs
