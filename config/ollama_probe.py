...
``tools`` — Pandora agents always emit tool calls. Best-effort:
never blocks proxy boot, just logs ``[pandora ollama]`` lines.
...
def tool_capability(
    ...
    *,
    ...
    return ProbeResult(
        ...
        f"(reported: {capabilities}). Pandora agents always emit "
