...
def _replace_config_arg() -> None:
    ...
    corresponding ``PANDORA_AUTH_*`` flag is enabled, even if no
    ``PANDORA_MODEL*`` override is set. Without this second branch a
    ...
    for idx, arg in enumerate(sys.argv):
        if arg == "--config" and idx + 1 < len(sys.argv):
            ...
            generated = write_dynamic_config(
                config_path,
                "/tmp/pandora-litellm/config.generated.yaml",
            )
    ...
    print(f"[pandora] registered dynamic config: {', '.join(parts)}", flush=True)
...
def _probe_ollama_if_configured() -> None:
    ...
    try:
        ...
        for line in probe(base, models):
            print(f"[pandora ollama] {line}", flush=True)
    except Exception as exc:  # noqa: BLE001
        # Observability-only — never let a probe bug crash proxy boot.
        print(f"[pandora ollama] probe failed unexpectedly: {exc}", flush=True)
...
print(
    "[pandora] auth dispatcher (claude_code, codex_chatgpt) + "
