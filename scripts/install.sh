...
#   PANDORA_HOME      — Install directory (default: ~/.pandora)
...
# ── Constants ─────────────────────────────────────────────────────
REPO="debasishtripathy13/Pandora"
BRANCH="${BRANCH:-main}"
...
download_files() {
    ...
    # PANDORA_HOME points at the current install dir.
    if [[ -f "$install_dir/.env" ]]; then
        if ! grep -q "^PANDORA_HOME=" "$install_dir/.env" 2>/dev/null; then
            echo "PANDORA_HOME=$install_dir" >> "$install_dir/.env"
        fi
        info ".env already exists, preserving your configuration."
    else
        info "No .env yet — run 'pandora onboard' to create one."
    fi
    ...
    # Version marker
    echo "$PANDORA_VERSION" > "$install_dir/.version"
}
...
create_launcher() {
    ...
    local binary_name="pandora-${os}-${arch}"

    if [[ "$PANDORA_VERSION" == "latest" ]]; then
        ...
        error "Example: VERSION=<x.y.z> curl -fsSL https://pandora.red/install | bash"
        exit 1
    fi

    local download_url="https://github.com/$REPO/releases/download/v${PANDORA_VERSION}/${binary_name}"
    info "Downloading launcher binary ($binary_name)..."
    if ! curl -fsSL "$download_url" -o "$bin_dir/pandora" 2>/dev/null; then
        error "No launcher binary for ${os}/${arch} in v${PANDORA_VERSION}."
    ...
    chmod 755 "$bin_dir/pandora"
}

# ── Detect stale `pandora` in PATH ─────────────────────────────
# A previous install via `npm link`, manual symlink, or alternate package
# manager can leave a `pandora` executable elsewhere on PATH. That stale
...
detect_stale_launcher() {
    ...
    if [[ ${#found[@]} -gt 0 ]]; then
        echo ""
        warn "Found other 'pandora' executable(s) on PATH:"
        for f in "${found[@]}"; do
            echo "  $f"
        done
        warn "These may shadow the launcher just installed at $bin_dir/pandora."
...
pull_images() {
    ...
    # inject PANDORA_VERSION/HOME explicitly so the image tags resolve.
    (cd "$install_dir" && \
        PANDORA_VERSION="$PANDORA_VERSION" \
        PANDORA_HOME="$install_dir" \
        docker compose --profile cli pull) || {
        warn "Warning: Failed to pull some images."
        info "You can pull them manually later: pandora update"
    }
}

# ── Main ──────────────────────────────────────────────────────────
main() {
    local install_dir="${PANDORA_HOME:-$HOME/.pandora}"
    local bin_dir="$HOME/.local/bin"


    echo ""
    echo -e "${BOLD}Pandora${NC} — Installer"
    echo ""
    ...
    info "Installing Pandora $PANDORA_VERSION"
    ...
    success "Launcher installed to $bin_dir/pandora"
    ...
    echo ""
    echo -e "${GREEN}────────────────────────────────────────────${NC}"
    echo -e "${GREEN}  Pandora installed successfully!${NC}"
    echo -e "${GREEN}────────────────────────────────────────────${NC}"
    echo ""
    echo -e "  ${BOLD}1.${NC} Configure your API keys:"
    echo -e "     ${BOLD}pandora onboard${NC}"
    echo ""
    echo -e "  ${BOLD}2.${NC} Start Pandora:"
    echo -e "     ${BOLD}pandora${NC}"
    echo ""
    ...
    #       shell hasn't sourced it yet → `pandora` not found.
    #   (b) The shell already has $bin_dir on PATH but a stale `pandora`
