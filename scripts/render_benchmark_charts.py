...
DIFFICULTY = {
    ...
    "Pandora": [100.0, 98.0, 87.5],  # L2 sweep ongoing (1 fail), L3 ongoing (1 fail)
}
LEVELS = ["L1 (Easy)", "L2 (Medium)", "L3 (Hard)"]

# Pandora — all-level pass/fail (L1+L2+L3 combined view).
...
# Pandora attack-class coverage — L1 + L2 + L3 totals.
COVERAGE = [
...
def chart_leaderboard() -> Path:
    ...
    fig.text(
        0.01,
        0.01,
        "Shannon: white-box, hint-removed.  Pandora: black-box, vulnerability tags as hint — 102 / 104.",
        fontsize=8,
        style="italic",
        color="#555",
    )
...
def chart_difficulty() -> Path:
    ...
    palette = {"Strix": "#3498db", "PentestGPT": "#9b59b6", "Pandora": US_COLOR}
    ...
    ax.set_title("Pass Rate by Difficulty — Strix · PentestGPT · Pandora")
...
# ---------------------------------------------------------------------------
# 3) Pandora donut by difficulty.
# ---------------------------------------------------------------------------


def chart_pandora_donut() -> Path:
    labels = [r[0] for r in PANDORA_PIE]
    sizes = [r[1] for r in PANDORA_PIE]
    colors = [r[2] for r in PANDORA_PIE]
    total = sum(sizes)
    passed = sum(n for lab, n, _ in PANDORA_PIE if "Not solved" not in lab)
    ...
    ax.set_title(
        "Pandora — XBOW Validation Benchmark",
        fontsize=14,
        fontweight="bold",
        pad=18,
    )
    ...
    return save(fig, "pandora_donut.png")


# ---------------------------------------------------------------------------
# 4) Pandora attack-class coverage.
# ---------------------------------------------------------------------------
...
def chart_coverage() -> Path:
    ...
    ax.set_title("Pandora — Web Attack Class Coverage on XBOW (L1 + L2 + L3)")
    ax.grid(axis="x", linestyle=":", alpha=0.4)
    fig.text(
        0.01,
        0.01,
...
def main() -> None:
    for fn in (chart_leaderboard, chart_difficulty, chart_pandora_donut, chart_coverage):
        path = fn()
