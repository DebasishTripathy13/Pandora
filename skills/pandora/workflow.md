---
name: pandora-workflow
description: "Pandora orchestrator workflow — engagement intake, OPPLAN build, execution loop via task() delegation, final report. Tools=[]; everything ships through sub-agents."
metadata:
  when_to_use: "pandora, orchestrator, engagement loop, OPPLAN, kill chain, delegate, task(), final report, executive summary"
  subdomain: workflow
---
...
1. On session start, ALWAYS run the `engagement-startup` skill (`load_skill("/skills/pandora/engagement-startup/SKILL.md")`).
...
1. Load the `final-report` skill (`load_skill("/skills/pandora/final-report/SKILL.md")`).
