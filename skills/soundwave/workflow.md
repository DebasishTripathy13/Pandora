...
description: "Soundwave planning agent workflow — interview, generate RoE / threat profile / CONOPS / OPPLAN, hand off to pandora."
metadata:
...
Before handing off to pandora, confirm:
...
### Phase 4 — Handoff (to Pandora)

1. Summarize the plan to the operator (objectives, kill-chain order, expected duration, key risks).
2. Notify pandora that the four artifacts are ready in `/workspace/plan/`. Pandora's engagement-startup skill picks up from there.
...
- **Re-plan when blocked.** If pandora reports an objective permanently BLOCKED, soundwave returns to Phase 2 to amend CONOPS/OPPLAN — never let the engagement stall silently.
