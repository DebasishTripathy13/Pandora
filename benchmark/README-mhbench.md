...
[MHBench](https://github.com/debasishtripathy13/MHBench) topology orchestrator into
Pandora's benchmark harness so Pandora agents can be scored against
...
- **Category 2 — Pandora side.** Provider-side automation. The provider
...
machine, not inside the Pandora container stack.**
...
The submodule is pinned to the `pandora` branch of
`debasishtripathy13/MHBench`. As of PR 1 the branch has zero patches on top of
upstream `bsinger98/MHBench@main` — it exists as a stable place to land
Pandora-side adjustments later.
...
What must be filled for Pandora's PR 1 scoring path:
...
- **`elastic_config` / `c2_config`** — placeholders are OK. Pandora
...
still returns 0. Pandora does not use the Caldera C2 channel, so
...
## Category 2 — Pandora side (what the provider does automatically)
...
4. **SSH key staging** — the operator's private key (from
   ...
   workspace at `~/.pandora/workspace/benchmark-<id>/perry_key` with
...
7. **Pandora agent runs the engagement.** Same harness path as
