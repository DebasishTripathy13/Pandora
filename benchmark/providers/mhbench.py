...
@dataclass(frozen=True)
class TopologySpec:
    """Pandora-side metadata for one upstream MHBench topology.
...
def _load_generated_topologies(submodule_dir: Path) -> dict[str, TopologySpec]:
    ...
    Pandora provider needs (foothold name, victim host name
...
class MHBenchProvider(BaseBenchmarkProvider):
    ...
    Pandora delegates topology lifecycle (setup / teardown) to MHBench's
    ...
    Pandora's evaluator pattern-matches that flag in agent output the
    ...
    **Topology-agnostic.** All per-topology data (env_type, victim name
    ...
    Adding a new MHBench topology to Pandora's benchmark suite is a
    ...
    def setup(self, challenge: Challenge) -> SetupResult:
        ...
        # agent's ssh_config. Use this when Pandora's stack runs on a
    ...
    def _classify_servers(
        ...
        without adding a Pandora-side dep. The snippet does pure
    ...
    def _stage_ssh_key(self, config_abs: Path, challenge_id: str) -> Path:
        ...
        The sandbox container bind-mounts ``~/.pandora/workspace/`` to
...
def _workspace_root() -> Path:
    return (Path.home() / ".pandora" / "workspace").resolve()
...
class ExternalTopology:
    ...
    Use when Pandora's stack runs on a different machine than the
