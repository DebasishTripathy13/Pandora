...
class Harness:
    """Runs benchmark challenges through the pandora main agent.

    The pandora agent handles the full kill chain:
    ...
    async def _query_cost(self, start_iso: str, end_iso: str) -> float | None:
        ...
        """
        master_key = os.getenv("LITELLM_MASTER_KEY", "sk-pandora-master")
        try:
    ...
    def _force_restart_langgraph(self, active: _ActiveRun) -> None:
        ...
        for net in ("benchmark_pandora-net", "benchmark_sandbox-net"):
            subprocess.run(
                ["docker", "network", "connect", net, "pandora-langgraph"],
                capture_output=True,
                check=False,
            )
        ...
        for attempt in range(30):
            try:
                r = httpx.get(
                    ...
                    headers={"Authorization": "Bearer sk-pandora-master"},
                    timeout=5,
                )
                if r.status_code == 200:
    ...
    def _reset_sandbox_state(self) -> None:
        ...
        for net in ("benchmark_pandora-net", "benchmark_sandbox-net"):
            subprocess.run(
                ["docker", "network", "connect", net, "pandora-sandbox"],
                capture_output=True,
                check=False,
            )
        ...
        for attempt in range(40):
            r = subprocess.run(
                ["docker", "exec", "pandora-sandbox", "true"],
                capture_output=True,
                check=False,
            )
            if r.returncode == 0:
    ...
    async def run_challenge(self, challenge: Challenge) -> ChallengeResult:
        # Use ~/.pandora/workspace/ which is bind-mounted as /workspace/ in the sandbox
        workspace = (Path.home() / f".pandora/workspace/benchmark-{challenge.id}").resolve()
        ...
        subprocess.run(
            ["docker", "exec", "pandora-sandbox", "rm", "-rf", sandbox_ws],
            capture_output=True,
        )
        ...
        ws_root = workspace.parent  # ~/.pandora/workspace
        if ws_root.exists():
        ...
        try:
            ...
            # Invoke pandora main agent — handles full chain via SubAgentMiddleware
    ...
    async def _invoke_agent(
        ...
        *,
        ...
        """Invoke the pandora main agent to execute one benchmark run.
        ...
        """
        # The sandbox maps ~/.pandora/workspace/ → /workspace/
