...
// Config holds the Pandora launcher configuration.
type Config struct {
	Home string
	Env  map[string]string
}

// PandoraHome returns the resolved PANDORA_HOME path.
func PandoraHome() string {
	if h := os.Getenv("PANDORA_HOME"); h != "" {
		return h
	}
...
func EnvPath() string {
	return filepath.Join(PandoraHome(), EnvFileName)
}
...
// with pandora/llm/factory.py::_API_METHOD_ENV.
...
func ValidateAPIKeys(env map[string]string) error {
	...
	msg.WriteString("\nRun 'pandora onboard --reset' to reconfigure credentials.")
...
//   - ChatGPT uses Pandora's auth/ handler reading the Codex CLI
...
type subscriptionMethod struct {
	Toggle                string   // PANDORA_AUTH_<X> boolean enabling this path
...
//
// OAuth paths:
//   - PANDORA_AUTH_CLAUDE_CODE=true requires a parseable
//     ~/.claude/.credentials.json. LiteLLM mounts that file read-only.
//   - PANDORA_AUTH_<X>=true (CHATGPT, GEMINI, COPILOT, GROK,
...
//
// Local LLM path: ollama_local in PANDORA_AUTH_PRIORITY (or any
...
func ValidateAuth(env map[string]string) error {
	claudeOAuth := isTruthy(Get(env, "PANDORA_AUTH_CLAUDE_CODE", ""))
...
func hasOllamaSelected(env map[string]string) bool {
	priority := strings.ToLower(strings.TrimSpace(env["PANDORA_AUTH_PRIORITY"]))
...
// either listed ollama_local in PANDORA_AUTH_PRIORITY or set
	...
	if base == "" {
		return fmt.Errorf(
			"ollama_local selected but OLLAMA_API_BASE is empty.\n" +
				...
				"in ~/.pandora/.env, or run 'pandora onboard --reset' to reconfigure.",
		)
	}
	return nil
}
...
func validateSubscriptionCredentials(env map[string]string, sub subscriptionMethod) error {
	...
	return fmt.Errorf(
		"%s subscription token not configured.\n"+
			...
			"  - %s env var\n"+
			"  - %s on disk\n"+
			"Run 'pandora onboard --reset' to (re)configure interactively.",
