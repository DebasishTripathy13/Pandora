...
`
	...
	if env["PANDORA_MODEL_PROFILE"] != "eco" {
		t.Errorf("PANDORA_MODEL_PROFILE = %q, want %q", env["PANDORA_MODEL_PROFILE"], "eco")
	}
	if len(env) != 4 {
...
func TestValidateAuth_OAuthSubscriptions(t *testing.T) {
	cases := []struct {
		...
		{"PANDORA_AUTH_GEMINI", "GEMINI_SESSION_COOKIES", "gemini", "tokens.json"},
		{"PANDORA_AUTH_COPILOT", "COPILOT_REFRESH_TOKEN", "copilot", "tokens.json"},
		{"PANDORA_AUTH_GROK", "GROK_SESSION_TOKEN", "grok", "tokens.json"},
		{"PANDORA_AUTH_PERPLEXITY", "PERPLEXITY_SESSION_TOKEN", "perplexity", "tokens.json"},
	}
	for _, c := range cases {
...
func TestValidateAuth_ChatGPTNativeOAuth(t *testing.T) {
	t.Run("toggle on allows native device login without session cookie", func(t *testing.T) {
		home := t.TempDir()
		t.Setenv("HOME", home)
		env := map[string]string{"PANDORA_AUTH_CHATGPT": "true"}
		if err := ValidateAuth(env); err != nil {
...
func TestValidateAuth_OllamaLocal(t *testing.T) {
	...
	t.Run("priority alone without base fails with helpful message", func(t *testing.T) {
		env := map[string]string{"PANDORA_AUTH_PRIORITY": "ollama_local"}
		err := ValidateAuth(env)
		if err == nil {
...
func TestValidateAuth_OAuth(t *testing.T) {
	...
	env := map[string]string{"PANDORA_AUTH_CLAUDE_CODE": "true"}
...
`
	...
	if env["PANDORA_MODEL_PROFILE"] != "max" {
		t.Errorf("PANDORA_MODEL_PROFILE = %q, want %q", env["PANDORA_MODEL_PROFILE"], "max")
	}
}

func TestPandoraHome(t *testing.T) {
	// With PANDORA_HOME set
	t.Setenv("PANDORA_HOME", "/custom/path")
	if got := PandoraHome(); got != "/custom/path" {
		t.Errorf("PandoraHome() = %q, want /custom/path", got)
	}

	// Without PANDORA_HOME — falls back to ~/.pandora
	t.Setenv("PANDORA_HOME", "")
	home := PandoraHome()
	if !filepath.IsAbs(home) {
		t.Errorf("PandoraHome() = %q, want absolute path", home)
	}
}
