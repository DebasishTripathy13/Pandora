...
var (
	...
	// still be compiling — this functional probe waits for the pandora
...
// pandora assistant. Compose's healthcheck only verifies that /ok serves;
...
//
// PANDORA_STARTUP_TIMEOUT_SECONDS overrides LangGraphTimeout for slow
	...
	for time.Now().Before(deadline) {
		...
		if err == nil {
			if resp.StatusCode == http.StatusOK {
				...
				resp.Body.Close()
				if bytes.Contains(respBody, []byte(`"pandora"`)) {
					return nil
				}
			} else {
				resp.Body.Close()
...
// pandora graph finishes compiling, so we probe /assistants/search until
