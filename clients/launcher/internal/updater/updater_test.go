...
func TestFetchLatestRelease_Mock(t *testing.T) {
	release := Release{
		TagName: "v1.2.0",
		Assets: []Asset{
			{Name: "pandora-linux-amd64", BrowserDownloadURL: "https://example.com/binary"},
		},
	}
	...
	if len(got.Assets) != 1 || got.Assets[0].Name != "pandora-linux-amd64" {
...
func TestSelfUpdate_NoMatchingAsset(t *testing.T) {
	...
	release := &Release{
		TagName: "v9.9.9",
		Assets:  []Asset{{Name: "pandora-plan9-mips", BrowserDownloadURL: "http://localhost/plan9"}},
	}
...
func TestSelfUpdate_WritesAndRenames(t *testing.T) {
	...
	dir := t.TempDir()
	fakeBin := filepath.Join(dir, "pandora")
