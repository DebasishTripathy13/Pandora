...
func SelfUpdate(release *Release) error {
	assetName := fmt.Sprintf("pandora-%s-%s", runtime.GOOS, runtime.GOARCH)
...
func WriteVersion(version string) error {
	versionFile := filepath.Join(config.PandoraHome(), ".version")
...
// users apply updates explicitly with `pandora update`.
//
...
func NotifyIfUpdateAvailable(currentVersion string) bool {
	...
	ui.DimText("Run `pandora update` to upgrade.")
	return true
}
...
// ``pandora update`` command and the interactive launch-time prompt.
//
...
func PromptIfUpdateAvailable(currentVersion string) (bool, error) {
	...
	if err := huh.NewConfirm().
		...
		Description(
			"Updates the launcher binary, docker-compose config, and pulls\n"+
				"the matching Docker images. Pandora will restart with\n"+
	...
	if !confirmed {
		ui.DimText(
			"Continuing with " + displayVersion(currentVersion) +
				". Run `pandora update` later to upgrade.",
		)
		return false, nil
	}

	// Determine the ref — same logic as ``pandora update``: prefer
	// the explicit branch override in .env, fall back to the release tag.
	ref := release.TagName
	if config.EnvExists() {
		if env, lerr := config.LoadEnv(config.EnvPath()); lerr == nil {
			if branch := strings.TrimSpace(env["PANDORA_BRANCH"]); branch != "" {
				ref = branch
			}
		}
	}
	...
	if err := reexecSelf(); err != nil {
		...
		ui.DimText("Run `pandora` again to use the new version.")
		os.Exit(0)
	}
