...
import (
	...
	"github.com/debasishtripathy13/Pandora/clients/launcher/internal/engagement"
	"github.com/debasishtripathy13/Pandora/clients/launcher/internal/health"
	"github.com/debasishtripathy13/Pandora/clients/launcher/internal/platform"
	...
	"github.com/debasishtripathy13/Pandora/clients/launcher/internal/updater"
...
var startCmd = &cobra.Command{
	Use:   "start",
	Short: "Start Pandora services and launch the CLI",
	RunE:  runStart,
}
...
func runStart(cmd *cobra.Command, args []string) error {
	...
	if _, err := os.Stat(composePath); os.IsNotExist(err) {
		...
		if version == "dev" || version == "" {
			ref = config.Get(env, "PANDORA_BRANCH", "main")
		}
	...
	// Ensure PANDORA_HOME is set in .env (Docker Compose needs absolute path)
	if config.Get(env, "PANDORA_HOME", "") == "" {
		env["PANDORA_HOME"] = home
		if err := config.AppendEnvLine(config.EnvPath(), "PANDORA_HOME", home); err != nil {
			ui.Warning("Could not set PANDORA_HOME in .env: " + err.Error())
		}
	}
	...
	// ack file at $PANDORA_HOME/.starred suppresses the prompt
	...
	// compose interpolates ${PANDORA_ENGAGEMENT_WORKSPACE} from this var.
	if err := os.Setenv("PANDORA_ENGAGEMENT_WORKSPACE", choice.WorkspacePath); err != nil {
	...
	ui.Info("Starting Pandora services...")
	...
	// 6. Launch CLI
	fmt.Println()
	ui.Info("Launching Pandora CLI...")

	cliEnv := map[string]string{
		"PANDORA_VERSION":      version,
		"PANDORA_ASSISTANT_ID": choice.AssistantID,
		"PANDORA_ENGAGEMENT":   choice.Engagement,
	}
	...
	// so re-entry is fast (cold start is ~75s); use 'pandora stop' to shut
	// the stack down.
	if err := c.RunInteractive(
		...
		ui.Warning("CLI exited with error — if services just started, try 'pandora' again.")
		ui.DimText("Run 'pandora logs litellm' or 'pandora logs langgraph' to debug.")
		return nil
	}

	ui.DimText("CLI exited. Services kept running — run 'pandora stop' to shut down.")
	return nil
}
...
func probeOllamaIfSelected(env map[string]string) {
	priority := strings.ToLower(env["PANDORA_AUTH_PRIORITY"])
