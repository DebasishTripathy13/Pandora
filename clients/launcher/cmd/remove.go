...
var removeCmd = &cobra.Command{
	...
	Short:   "Uninstall Pandora completely",
	RunE:    runRemove,
}
...
func runRemove(cmd *cobra.Command, args []string) error {
	...
	out, err := exec.Command("docker", "images", "--format", "{{.Repository}}:{{.Tag}}", "--filter", "reference=*pandora*").Output()
	if err == nil {
	...
	backupDir := filepath.Join(userHome, "pandora-workspace-backup")
	...
	if preserveWorkspace {
		...
		if err := backupWorkspace(wsDir, backupDir); err != nil {
			...
			ui.DimText("Re-run 'pandora remove' or move the workspace manually before deleting " + home)
	...
	ui.Success("Pandora has been removed")
	if preserveWorkspace {
...
// `pandora` AND `.local/bin` on the same line, which never matched
...
func cleanPathFromFile(path string) {
	...
	for i < len(lines) {
		line := lines[i]
		if strings.TrimSpace(line) == "# pandora" && i+1 < len(lines) && isInstallPathLine(lines[i+1]) {
