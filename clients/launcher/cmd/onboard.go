...
import (
	...
	"charm.land/huh/v2"
	"github.com/debasishtripathy13/Pandora/clients/launcher/internal/config"
	"github.com/debasishtripathy13/Pandora/clients/launcher/internal/starprompt"
...
var onboardCmd = &cobra.Command{
	Use:   "onboard",
	Short: "Configure Pandora (auth methods, model profile, observability)",
	RunE:  runOnboard,
}
...
// AuthMethod identifiers — must match pandora/llm/models.py::AuthMethod.
const (
...
// WSL2 distro, native Linux): from inside Pandora's containers it
...
// resulting PANDORA_AUTH_PRIORITY preserves this order, filtered
...
func runOnboard(cmd *cobra.Command, args []string) error {
	if config.EnvExists() && !resetFlag {
		...
		ui.DimText("Run 'pandora onboard --reset' to reconfigure")
		return nil
	}
	...
	).WithTheme(huh.ThemeFunc(ui.PandoraTheme))
	...
	values := map[string]string{
		"PANDORA_MODEL_PROFILE":    profile,
		"PANDORA_LANGUAGE":         language,
		"PANDORA_AUTH_PRIORITY":    strings.Join(priority, ","),
		"PANDORA_AUTH_CLAUDE_CODE": boolStr(contains(methods, methodAnthropicOAuth)),
		"PANDORA_AUTH_CHATGPT":     boolStr(contains(methods, methodOpenAIOAuth)),
		"PANDORA_AUTH_GEMINI":      boolStr(contains(methods, methodGoogleOAuth)),
		"PANDORA_AUTH_GROK":        boolStr(contains(methods, methodGrokOAuth)),
		"PANDORA_AUTH_COPILOT":     boolStr(contains(methods, methodCopilotOAuth)),
		"PANDORA_AUTH_PERPLEXITY":  boolStr(contains(methods, methodPerplexityOAuth)),
	}
	...
	if useLangSmith && langSmithKey != "" {
		...
		values["LANGSMITH_PROJECT"] = "pandora"
	}
	...
	// $PANDORA_HOME/.starred, so a re-run of `pandora onboard
	// --reset` does not re-prompt.
	starprompt.PromptIfNotStarred()

	ui.DimText("  Run 'pandora' to start the platform")
	return nil
}
	...
	if probe.Reachable {
		return fmt.Errorf(
			"Ollama selected but no tool-capable models found on the host.\n" +
				"Pandora agents always emit tool calls — pull a tool-capable\n" +
				"model and verify it advertises tools, then re-run:\n\n" +
				"  ollama pull qwen3-coder:30b\n" +
				"  ollama show qwen3-coder:30b   # capabilities should list 'tools'\n" +
				"  pandora onboard --reset")
	}
	return fmt.Errorf(
