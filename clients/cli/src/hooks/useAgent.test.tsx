...
// useAgent reads INITIAL_ASSISTANT_ID = process.env.PANDORA_ASSISTANT_ID at
...
describe("useAgent — engagement handoff lifecycle", () => {
  beforeEach(async () => {
    vi.resetModules();
    vi.useFakeTimers();
    vi.stubEnv("PANDORA_API_URL", "http://localhost:2024");
    vi.stubEnv("PANDORA_ASSISTANT_ID", "soundwave");
    vi.stubEnv("PANDORA_ENGAGEMENT", "eng-abc");
    vi.stubEnv("PANDORA_WORKSPACE_PATH", "/tmp/ws");
    delete process.env.PANDORA_THREAD_ID;
  ...
  it("flips assistantId to 'pandora' when engagement_ready fires mid-stream", async () => {
    ...
    expect(result.current.assistantId).toBe("pandora");
  ...
  // ── 2. Handoff auto-submits enqueued message on fresh pandora thread ───
  it("auto-submits enqueued message on a fresh pandora thread after handoff", async () => {
    ...
    (mc.threads.create as Mock)
      ...
      .mockResolvedValueOnce({ thread_id: "thread-pandora" });
    ...
    // New thread created for the pandora run
    expect(createCalls.length).toBe(2);

    // Second stream call targets pandora assistant on the fresh thread
    expect(streamCalls.length).toBe(2);
    expect(streamCalls[1][0]).toBe("thread-pandora"); // new thread, not original
    expect(streamCalls[1][1]).toBe("pandora");
  ...
  it("injects engagement context via config.configurable only — never as top-level run input", async () => {
    ...
    act(() => {
      result.current.enqueue("pandora prompt");
    });
  ...
  it("keeps queuedMessage intact after engagement_ready — only handleStreamComplete handoff branch clears it", async () => {
    ...
    (mc.threads.create as Mock)
      ...
      .mockResolvedValueOnce({ thread_id: "t-pandora" });
