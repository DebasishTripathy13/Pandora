...
interface UseAgentReturn {
  ...
  /** Currently active agent name (e.g. "pandora", "recon"). */
  activeAgent: string | null;
  /** Persistent assistant id ("soundwave" | "pandora") — shown when no subagent is streaming. */
  assistantId: string;
...
// - "pandora" for resuming an existing engagement
// Defaults to "pandora" when launched directly (legacy / dev workflows).
//
// When soundwave finishes its interview and emits the `engagement_ready`
// custom event, the active assistant is flipped in-flight to "pandora"
// and the next operator message starts a fresh thread on that assistant —
// no CLI restart needed.
const INITIAL_ASSISTANT_ID =
  process.env.PANDORA_ASSISTANT_ID || "pandora";

export function useAgent({
  apiUrl = process.env.PANDORA_API_URL || "http://localhost:2024",
  resumeThread = false,
  ...
  const envThreadId = process.env.PANDORA_THREAD_ID || null;
  ...
  // tool flips this to "pandora" mid-flight; the next submit() then opens
  ...
  // opens a fresh pandora thread. Carries no slug; the launcher is the single
      ...
      const handleCustomEvent = (data: SubagentCustomEvent) => {
        switch (data.type) {
          ...
          case "engagement_ready": {
            ...
            assistantIdRef.current = "pandora";
            setAssistantId("pandora");
            addEvent({
              type: "system",
              content:
                "Engagement planning complete — Pandora will pick up your next message.",
            });
            break;
          }
  ...
  const handleStreamComplete = useCallback(
    (abortController: AbortController) => {
      ...
      // flipped assistantIdRef to "pandora" during this run. Drop the
      // soundwave thread so the next submit opens a fresh pandora
  ...
  const submit = useCallback(
    (message: string): void => {
      ...
      const runStream = async () => {
        ...
        const slug = process.env.PANDORA_ENGAGEMENT;
        if (slug) {
          configurable.engagement_name = slug;
          configurable.workspace_path =
            process.env.PANDORA_WORKSPACE_PATH ?? "/workspace";
        }
