...
/**
 * useRunObserver — real-time observation of LangGraph runs via @pandora/streaming.
 *
...
import { type SubagentCustomEvent, STREAM_OPTIONS } from "@pandora/streaming";
    ...
    try {
      ...
      for await (const event of stream) {
        ...
        if (event.event === "custom") {
          ...
          // Extract orchestrator (pandora) tool calls from message history
          ...
          if (data?.messages) {
            ...
            if (lastMsg?.type === "ai" && lastMsg.tool_calls?.length) {
              for (const tc of lastMsg.tool_calls) {
                ...
                if (!seenToolCalls.has(`pandora-${tc.name}-${data.messages.length}`)) {
                  seenToolCalls.add(`pandora-${tc.name}-${data.messages.length}`);
                  newEvents.push({
              ...
              if (lastMsg.name !== "task") {
                const key = `pandora-${lastMsg.name}-result-${data.messages.length}`;
