#!/usr/bin/env node
/**
 * Terminal WebSocket Server — spawns Pandora CLI in a PTY.
 *
...
async function createThread(engagementId: string, agentId: string): Promise<string> {
  ...
  try {
    const res = await fetch(`${LANGGRAPH_API_URL}/threads`, {
      ...
      body: JSON.stringify({ metadata: { engagement_id: engagementId, pandora_assistant: agentId } }),
...
wss.on("connection", async (ws: WebSocket, req) => {
  ...
  const env: Record<string, string> = {
    ...
    TERM: "xterm-256color",
    FORCE_COLOR: "1",
    PANDORA_ASSISTANT_ID: agentId,
    PANDORA_ENGAGEMENT: engagementSlug,
    PANDORA_WORKSPACE_PATH: engagementSlug ? `/workspace/${engagementSlug}` : "/workspace",
    PANDORA_API_URL: LANGGRAPH_API_URL,
  };
  if (threadId) env.PANDORA_THREAD_ID = threadId;
